...
debugged=[]
def find_deb(mistakes_kolvo_q,mistakes_q,array):
	for i in range(mistakes_kolvo_q):
		if mistakes_q[i] not in array or mistakes_q[i] in array and mistakes_q.count(mistakes_q[i])!=array.count(mistakes_q[i]) :
